...
<Tabs>
  ...
  | ------- | ----------- | ------- |
  | `@proto/data` | Dataset schema contracts, lifecycle governance, migration helpers | `createDataProtocol`, `createDataCatalog` |
  | `@proto/event` | Event bus contracts for Kafka/Kinesis/AsyncAPI | `createEventProtocol` |
  | `@proto/api` | API surface specifications with OpenAPI + SDK codegen | `createApiProtocol`, `createApiCatalog` |
  | `@proto/agent` | Agent capability manifests, guardrails, evaluation hooks | `createAgentProtocol` |
  | `@proto/semantic` | Semantic intent graph, similarity search, criticality scoring | `createSemanticProtocol` |
