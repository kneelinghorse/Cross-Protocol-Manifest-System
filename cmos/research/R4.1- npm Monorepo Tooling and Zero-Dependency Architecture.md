...
npm workspaces: This is the standard, built-in solution. It provides basic symlinking and a way to run scripts across packages. Its primary and problematic characteristic is its "flat" node_modules hoisting structure. This structure can lead to "phantom dependencies," where a package (@proto/core) can successfully import a library that was only installed as a dependency of a sibling package (@proto/data). This behavior encourages sloppy dependency management and is a direct threat to the zero-dependency philosophy.   
...
├── internal/
│   ├── utils/         # @proto/utils (shared code, devDependency only)
│   ├── eslint-config/ # @proto/eslint-config (dev tool)
│   └── tsconfig/      # @proto/tsconfig (shared TS base config)
├── packages/
...
Creation: The internal/utils package is created with its own package.json, defining its name as "@proto/utils".

Installation: A consuming package (e.g., packages/core) installs this utility as a devDependency:

Bash
pnpm add @proto/utils --filter @proto/core --save-dev
Local State: The packages/core/package.json now lists "@proto/utils": "workspace:*" under devDependencies. pnpm symlinks this package locally.

Bundling: The packages/core package uses tsup as its bundler.   

Build Execution: When turbo run build --filter @proto/core is executed, tsup builds @proto/core. It encounters import { sharedFunction } from '@proto/utils' in the source code.

Inlining: tsup's underlying philosophy (and that of esbuild) is that devDependencies are build-time tools, and their code should be inlined into the final bundle. It resolves the code from @proto/utils and bundles it directly into the output files (e.g., dist/index.js and dist/index.mjs) of @proto/core.   

Final Result: The published @proto/core package on npm contains the necessary utility code but has zero entries in its dependencies block. The zero-dependency constraint is successfully met.
...
export default defineConfig({
  ...
  // By default, tsup bundles all devDependencies (like @proto/utils)
...
Initial Setup: A developer runs pnpm install once at the root. pnpm reads the pnpm-workspace.yaml and symlinks all internal packages (e.g., @proto/utils becomes available to @proto/core).   

Running Tasks: All commands are run from the monorepo root using the turbo binary.

Build everything: turbo run build

Develop (watch) a single package: turbo run dev --filter=@proto/core

Build a package and its dependencies: turbo run build --filter=@proto/data (This will first build @proto/utils if it has changed, cache it, and then build @proto/data).   

Run tests: turbo run test (Turborepo pulls from cache for all packages with no changes).   

Changes made to internal/utils are immediately picked up by the tsup --watch process running for @proto/core, as pnpm has linked them. This provides a seamless, fast feedback loop.
...
A developer makes changes to @proto/core and the shared @proto/utils.

Before committing, they run: pnpm changeset add.

The CLI prompts: "Which packages? (select @proto/core)".
...
Pro: This is the correct model for public, modular libraries. @proto/core can be stable at v2.0.0 while @proto/data is iterating rapidly at v1.1.0. Consumers only update the protocols they need.

Con: This introduces the standard complexity of modular libraries: a consumer might use a v1.0.0 @proto/core with a v2.0.0 @proto/data that are incompatible.
...
[ ] Create internal/utils package, including its package.json (name: @proto/utils).
...
[ ] Refactor all import statements in packages/* to use the shared util (e.g., import {... } from '@proto/utils').
